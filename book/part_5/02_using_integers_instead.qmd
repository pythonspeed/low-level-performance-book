...
def mean_threshold_numpy(image):
    mean = image.mean()
    result = (image > mean).astype(np.uint8)
    result *= 255
    return result
...
As a first pass, we can write a implementation using Numba.
Because we're accumulating a large number of `uint8` to create the same, we need to store it in a larger value, so we'll use a `float64`:
...
On my computer the speed is essentially the same, we haven't gotten any improvement in speed:
