...
1. **Auto-vectorization:** Compilers can sometimes generate SIMD instructions automatically, especially if you structure your code appropriately.
   ...
   This is what we focused on in earlier chapters.
2. **Intrinsics:** You directly tell the compiler to use specific CPU instructions, using special compiler features called "intrinsics".
...
Notably, if you're compiled C/C++/Cython extensions for Python by default no auto-vectorization happens at all.
...
And sometimes your code will need to run on many different CPUs, so you can't even predict in advance exactly which SIMD instructions will be available!
...
| Does this support—      | —compiling in production? | —runtime dispatch?                        | —lowest common denominator? |
|-------------------------|---------------------------|-------------------------------------------|-----------------------------|
...
If you have some control over your hardware `x86-64-v3` or `x86-64-v4` may work.
