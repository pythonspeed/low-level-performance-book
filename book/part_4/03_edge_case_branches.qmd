...
Given most data won't be an edge choice, the branch choice will easy to predict.
...
Let's say we have a daily time series of values between 0 and 1, and we want to calculate the 7-day moving average.
For most values we just calculate the average of the previous (inclusive) 7 values.
But for the first few values we don't have 7 previous values.
...
In the previous chapter we saw how branchless code can replace branches, where the main benefit is getting rid of mispredicted branches.
We can do that here:
...
This isn't a great improvement, however, because the branch we just replaced is very predictable.
