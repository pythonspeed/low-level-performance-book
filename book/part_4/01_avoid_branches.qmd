...
Code with branches makes it harder for the CPU to generate efficient code:
...
## ILP, branch prediction, and the cost of misprediction
...
Based on this prediction, the CPU speculatively keeps executing in parallel _as if_ that prediction is correct.

* If the chosen branch turns out to be correct, all is well.
...
In practice, many conditionals have very consistent answers for long stretches of time, in which case branch prediction will end up being very accurate.
