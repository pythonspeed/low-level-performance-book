...
## Sometimes branchless programming will be slower

Branchless programming is a trade-off: in return for getting rid of the cost of branch misprediction, you pay the cost of calculating both branches, instead of just one.

TODO SIMD goes both ways. branchless might make it easier to generate SIMD code and then that will hopefully undo or prevent the cost of calculating both branches. Or auto-vectorization may be able to turn seemingly branching code into non-branching code use masking. explicit SIMD is another way to avoid branches.
...
## Beware branchless programming that has branches

Maybe? TODO Subtle point that we're relying on the compiler to notice the `A if Y else B` pattern it can execute both A and B. short circuiting typically means it won't. demonstrate how a complex/side-effecty A can break the code and how to work around it.
