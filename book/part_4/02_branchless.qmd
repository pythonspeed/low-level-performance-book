...
If we want to clean those areas up, a simplistic algorithm is setting all values below a certain threshold to complete darkness, i.e. 0.
...
Even though the function runs with almost the same number of CPU instructions, it runs much more slowly with the unpredictable, noisy image.
...
In this particular example, a branch is inherently difficult to predict for certain inputs.
...
Branchless programming is a trade-off: in return for getting rid of the _possible_ cost of branch misprediction, you pay the _definitive_ cost of calculating two or more branches, instead of just one.
