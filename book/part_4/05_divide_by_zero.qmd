...
* **Rust:** Integer division only, since it will cause a panic.
* **Numba:** Both floating point and integer division by zero will cause a `ZeroDivisionError` exception by default.
* **Cython:** Both floating point and integer division by zero will cause a `ZeroDivisionError` exception by default.
...
## Strategy #1: Get rid of the branch
...
* Integer division by zero returns zero, which is perhaps not what you'd expect.
  ...
  The Cython equivalent (`cdivision`) probably does help, but has a large number of semantic impacts so [read the documentation carefully](https://cython.readthedocs.io/en/latest/src/userguide/source_files_and_compilation.html#compiler-directives).
