...
As an example we'll using a moving average function again, but this time we'll just use zeros to fill in the missing first few values.
...
The general approach is to figure out idioms that are transparent enough to the compiler that it can prove to itself that the bounds will never be violated, or alternatively providing asserts that give it more information.
...
How about if we do `for value in arr:`?
This seems like it ought to avoid bounds checking altogether.
...
sum_for2_bc, sum_for2_no_bc = with_and_without_boundscheck(sum_for2)
assert sum_for2_bc(DATA) == sum_for2_no_bc(DATA)
```
...
Bounds checking has no cost... but now our function is much slower even with bounds checking disabled.
...
sum_for3_bc, sum_for3_no_bc = with_and_without_boundscheck(sum_for3)
assert sum_for3_bc(DATA) == sum_for3_no_bc(DATA)
```
