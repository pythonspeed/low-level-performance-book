...
Here's an example of two functions that seem identical, but behave differently when the inputs are aliased[^example]:

[^example]: This example was inspired by one in _Computer Systems: A Programmer's Perspective_, by Bryant and O'Hallaron.
...
@njit
def add_twice_cached(first_arr, second_arr):
    for i in range(len(first_arr)):
        value = second_arr[i]
        first_arr[i] += value
        first_arr[i] += value
...
def ones():
    return np.ones((1_000_000, ), dtype=np.uint64)
...
assert np.array_equal(
    add_twice(ones(), DATA2),
    add_twice_cached(ones(), DATA2)
)
```

The first implementation is slower, because it does more work and the compiler doesn't optimize that work away:

```{python}
#| echo: false
%%compare_timing

add_twice(DATA, DATA2)
add_twice_cached(DATA, DATA2)
```
...
arr = np.ones((3, ), dtype=np.uint64)
print("add_twice_cached() gives:", add_twice_cached(arr, arr))
```

So how do we deal with the fact that the _potential_ for aliasing can prevent optimization?

## Solution #1: Manual optimization

Rather than relying on the compiler, you can implement the optimizations yourself.
In our example, you could write the verison of the code in `add_twice_cached()` instead of `add_twice()`, assuming that matches the semantics you want in the case when the data is aliased.
...
In particular, the issue in the examples above was due to _writing_ to the passed-in array, which meant writes could impact future reads.
...
@njit
def add_twice_cached_new_array(first_arr, second_arr):
    result = np.empty(first_arr.shape, dtype=first_arr.dtype)
    for i in range(len(result)):
        value = second_arr[i]
        result[i] = first_arr[i] + value
        result[i] += value
    return result
...
ONES = ones()
assert np.array_equal(
    add_twice_new_array(ONES, DATA2),
    add_twice_cached_new_array(ONES, DATA2)
)
```
