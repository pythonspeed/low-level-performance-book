...
Compilers can't read your mind; they can only read your code.
And one key requirement for the compiler's optimization passes is that your code continues to have the _exact_ same behavior.
**To an outside observer, the results of running optimized code must be indistinguishable from running the original code.**

As a result, in cases where the optimization might change your code's behavior, tthe compiler won't apply seemingly obvious optimizations.
In this chapter we'll cover one of the problems that can prevent the compiler from optimizing your code: the potential for aliasing.

## Pointers and aliasing

We first need to cover a couple of definitions:
