...
To test our implementations, I generated a simulated image:
...
with disabled_simd() as njit_no_simd:
    ...
    denoised = remove_noise_naive(IMAGE, 1000)
```
...
with disabled_simd() as njit_no_simd:
    ...
    assert np.array_equal(
        denoised,
        remove_noise_branchless_1(IMAGE, 1000)
    )
```
...
with disabled_simd() as njit_no_simd:
    ...
    assert np.array_equal(
        denoised,
        remove_noise_branchless_2(IMAGE, 1000)
    )
```
...
with disabled_simd() as njit_no_simd:
    ...
    assert np.array_equal(
        denoised,
        remove_noise_branchless_3(IMAGE, 1000)
    )
```
...
On my computer, which has an x86-64 CPU with support for AVX2 SIMD instructions, the branchless versions get much faster with SIMD enabled.
A reader with a CPU that had AVX-512 actually got a huge speedup on the naive version too: the compiler was able to use SIMD instructions to remove the branch altogether.
It was probably using a SIMD instruction that supports masking, conditionally doing an operation.
A smarter compiler might be able to use SIMD with AVX2 as well.

Nonethelss, the more complex your code, the harder it will be for the CPU to automatically convert branches to masked SIMD instructions.
...
assert np.array_equal(
    count_sequences(RANDOM_DATA),
    count_sequences_branchless_1(RANDOM_DATA)
)
```
...
By getting rid of branches, we've signifcantly sped up the `RANDOM_DATA` case.
However, the `PREDICTABLE_DATA` case is actually slower.
Can you figure out why?
Hint: It's got nothing to do with branch prediction.

### Fixing our performance regression

The reason our latest version is slower on `PREDICTABLE_DATA` is that `result[INCREASE]` has become a data dependency.
Because it gets written to in every loop, the CPU can't use instruction-level parallelism to do multiple iterations of the loop in paralle.
For `RANDOM_DATA` this is less of a problem because loop iterations don't always write to the same place.

We can fix this by having multiple accumulators:
...
assert np.array_equal(
    count_sequences(RANDOM_DATA),
    count_sequences_branchless_2(RANDOM_DATA)
)
```
