...
We now have four channels, with different brightness values, one for each base; we pick the channel brightest (highest) value, which tells us which base this is.
We do this repeatedly for each item in the DNA sequence, until we've sequenced the whole ting.
...
We're going to implement the second phase, and optimize it so it's 20× faster than our initial implementation:
...
### Getting rid of branch mispredictions
...
The first problem seems more problematic, so let's fix it:
