...
Notice that instead of having a whole temporary `float64` array the size of the original image, we just have temporary `float64` values for one pixel at a time, massively reducing our memory usage.
