...
def parallel(arr, num_chunks=10):
    ...
    # 😎 For each chunk, process it in a thread in the thread pool by calling
...
Or perhaps you always write to the same location in the filesystem.

In general, if you expect your code to be called from multiple threads (which should be your default expectation) you should avoid writing to, modifying, or otherwise mutating any shared resource if you can avoid it.
If you can't avoid it, make sure you're doing it in a way that avoids race conditions by following one of the strategies above.
