...
# impact our benchmarks. By default it will start a number of threads equal to
# the number of cores.
THREAD_POOL = ThreadPoolExecutor()

def parallel_summer(arr, num_chunks=1000):
    result = np.array([0, 0], dtype=arr.dtype)
    ...
    def process_chunk(result, chunk):
        for i in range(len(chunk)):
            ...
            square = val ** 2
            cube = square * val
            # 😱 This is a bug, leading to incorrect results:
            result[0] += square
            result[1] += cube
    ...
    # 😎 For each chunk, process it in a thread in the thread pool:
...
def parallel_summer_locking(arr, num_chunks=1000):
    result = np.array([0, 0], dtype=arr.dtype)
    lock = Lock()

    @jit(nogil=True)
    def process_chunk_core(chunk):
        square = 0
        cube = 0
        for i in range(len(chunk)):
            val = chunk[i]
            # This is fine, we're adding to a local variable only accessed by
            # the current thread:
            square += val ** 2
            cube += square * val
        return square, cube
    ...
    def process_chunk(result, chunk):
        ...
        square, cube = process_chunk_core(chunk)
...
```{python}
#| echo: false
%%compare_timing --units=Numbers:len(DATA)
squares_and_cubes(DATA),
parallel_summer_locking(DATA)
```
...
This is a variation of the map/reduce pattern: the threads map a function over chunks of data without mutating any shared state, and then the partial results are reduced into a final result.

```{python}
def parallel_summer_mapreduce(arr, num_chunks=1000):
    @jit(nogil=True)
    def process_chunk(chunk):
        # This never writes to `result`:
        square = 0
        cube = 0
        for i in range(len(chunk)):
            val = chunk[i]
    ...
    result = np.array([0, 0], dtype=arr.dtype)
    for (part_square, part_cube) in THREAD_POOL.map(process_chunk, chunks):
        result[0] += partial_square
        result[1] += partial_cube
...
A third alternative is atomics
