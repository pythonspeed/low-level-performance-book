...
That means you should, as discussed in later chapters:

* [Avoid writing to shared data](020_race_conditions.qmd) so that running multiple copies in parallel won't conflict.
* Make sure you [release the GIL](030_gil.qmd) so multiple threads can run in parallel.
...
Both pipelines (when run inside a single process) and parallelism within functions typically involve the use of thread pools, so I'll discuss those in the next chapter.
