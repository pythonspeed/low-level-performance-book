...
In many parallel applications you will have access to a large amount of data, and process it in chunks in parallel.
...
Separately, a lower-overhead thread pool might also help.
...
My current computer's CPU has 12 cores, and a 25MiB L3 memory cache shared by all the cores.
Let's say that in order to utilize all the cores, I am running my code with 12 threads.

If each chunk I process in the thread pool is 10MiB of data, that means the threads are accessing at least 250MiB of data.
If the data access pattern is random, the immediately accessed data won't fit in the L3 cache and there will be lots of time spent waiting for reads from RAM.

On the other hand, if each chunk is 1MiB of data, the total data across all threads will be 12MiB, which will fit in the L3 cache with room to spare for other necessary memory.
This hopefully means faster calculations as less time will be spent waiting for reads from RAM.
...
While latency is always a problem, having multiple threads reading or writing large amounts of data from RAM makes it more likely you will hit the bandwidth limit.
...
def parallel_with_chunk_size(arr, chunk_size):
...
assert abs(parallel_with_chunk_size(DATA, 10_000_000) - 4.5) < 0.01
```

Both effects, overhead and memory pressure, show up when the code is benchmarked:

```{python}
#| echo: false
%%compare_timing --measure=memory_cache_miss
parallel_with_chunk_size(DATA, 100_000)
parallel_with_chunk_size(DATA, 1_000_000)
parallel_with_chunk_size(DATA, 10_000_000)
```

When the chunks are small and there are many chunks, the overhead slows down the operation.
When chunks are few but large, memory pressure starts slowing things down.
Peak performance is somewhere in the middle.

### Next steps {.unnumbered}

Having covered choosing the appropriate chunk size, in the next chapter you'll learn how to choose the appropriate number of threads in your thread pool.
