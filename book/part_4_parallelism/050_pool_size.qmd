...
Whenever a running thread that is only doing computation has had sufficient time, that thread is paused by the operating system and another thread is resumed so it can run on the newly freed up CPU core.
...
[^other]: There may be some minor processes, as well the operating system kernel itself, and they will need some CPU. But probably not much, so you can ignore them in practice.

On Python you can get the number of CPU cores using [`os.cpu_count()`](https://docs.python.org/3/library/os.html#os.cpu_count).
Here's how you can create a single thread pool, with the number of workers limited to the number of CPU cores:
...
In Python, the [`os.process_cpu_count()`](https://docs.python.org/3/library/os.html#os.process_cpu_count) API is an alternative to `os.cpu_count()` that takes into account CPU affinities.
...
The Rust crate [`num_cpus`](https://docs.rs/num_cpus/latest/num_cpus/) provides an API to retrieve the number of available CPU cores, taking into account both affinity and `cgroups`.
...
## Adjust libraries' thread pool sizes to match your parallelism strategy

Imagine you decided to add parallelism to a program by using parallel linear pipelines, each one running single-threaded.
You launch multiple processes in parallel, each processing a different file.
Given a computer with $C$ CPU cores, how many processes should you launch?
Based on the discussion above, you might choose to launch $C$ different processes.

But there's a potential problem here.
Many Python libraries will have their own internal thread pool, with the number of threads often defaulting to the number of CPU cores.
In fact, some libraries are inconsistent about their threading model, notably NumPy: most of the time it is single-threaded, but when doing linear algebra operations it uses multi-threaded BLAS library as a backend.
As a result, some (but not all!) of your operations may be using $C$ threads.
Given $C$ processes each with $C$ threads, overall you might have $C^2$ different threads running in parallel, which is going to slow things down.

To prevent this from happening, you should make sure any libraries you depend on are configured appropriately to match your application's parallelism strategy.
If your application will manage the amount of parallelism, set the libraries' thread pool sizes to 1.
If you want to rely on the libraries' existing parallelism, don't implement parallelism yourself.

### Next steps {.unnumbered}

So far the book has covered ways to change your code to make it faster.
Next you'll learn about ways to speed up your code—or at least, keep it from getting slower—using the processes surrounding your coding.
