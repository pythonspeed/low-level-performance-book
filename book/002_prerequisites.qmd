...
Nonetheless, there is a little bit of knowledge you will need to understand the examples in later chapters, which I'll quickly cover here.

TODO figure out how much of this can be shortened, deleted, or moved to later chapters
...
TODO do we want to cover this?

## A quick intro to Numba

The concepts in this book should apply to C, C++, Rust, and other low-level languages.
However, to make it easy to follow the code, the examples will all use Numba.
...
That makes it an excellent language for teaching about low-level programming, since you don't need to learn a new programming language or a new development environment.
...
In particular, it compiles any function decorated with the `numba.njit` function:
...
The first time you call a function decorated with `@njit` with a specific set of typed parameters, Numba will compile the function.
...
> **Note:** Compilation is done _before_ the function is run, so while it says "JIT" in the decorator name, this is not a just-in-time compiler in the way that PyPy, JavaScript, or Java JVMs are.
> JIT compilers compile the code after observing it actually running.
> Numba, in contrast, only looks at the source code of the function and the types of its arguments in order to compile it, so it's really an ahead-of-time compiler as is used for C++ or Rust.
> When it talks about just-in-time compilation, this is a description of the developer experience, not the compiler technique.
...
When you call a NumPy API inside a function decorated with `numba.njit`, you are not actually calling the NumPy API.
