...
As we discussed in the previous chapter, ILP allows the CPU to run multiple instructions in parallel if it knows they don't depend on each other.
...
# other effects; SIMD will be discussed in the next chapter.
import os
...
Instruction-level parallelism has the CPU core running standard CPU instructions in parallel, on its own, automatically.
Another option is for the compiler to explicitly tell the CPU core to use special CPU instruction that operate on multiple pieces of data in parallel.
This is known as SIMD, the topic of the next chapter.
