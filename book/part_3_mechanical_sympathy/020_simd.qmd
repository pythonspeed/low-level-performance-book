...
Single Instruction Multiple Data, or SIMD, are a type of CPU instructions that can execute the same operation on multiple primitive values (integers or floats) at once.
The compiler needs to explicitly generate these special instructions, unlike the instruction-level parallelism covered in the previous chapter.

For example, if you want to multiply 4 integers by a constant, you can do that with a single SIMD CPU instruction instead of with 4 normal CPU instructions.
...
Given this a book for Python developers, what "vectorization" means can be ambiguous, so make sure you understand which of the two meaning applies when reading documentation.
:::
...
In the previous chapter we discussed instruction-level parallelism, where the CPU core runs multiple instructions in parallel.
SIMD is also a way to execute work in parallel.
So what is the difference?

* **Instruction-level parallelism:** The decision to use this happens at runtime, and is made by the CPU.
* **SIMD:** The decision to use this happens when the code is compiled.
    Only if the compiler generate specialized SIMD machine code instructions will SIMD be used at runtime.
...
```
simd_add256 a[0:4] b[0:4] → out[0:4]
```
...
I'll talk about how the batch size is decided in just a bit, but first let's see how SIMD speeds up some example code.
In particular, I will compile two versions of the exact same code, one compiled with SIMD (the default) and one with SIMD disabled:
...
* For 32-bit floats, $256 / 32 = 8$, so the CPU core can process 8 `float32` with each 256-bit SIMD instruction.
  $1,000,000 / 8 = 125,000$ instructions.
* For 64-bit floats, $256 / 64 = 4$, so the CPU core can process 4 `float64` with each 256-bit SIMD instruction.
  Given an array of 1,000,000 64-bit values, processing it will take $1,000,000 / 4 = 250,000$ instructions.

Notice that these numbers match what I empirically measured in the previous section!
...
* **Branches.** SIMD instructions do the same thing to multiple pieces of data, which means branching for each item may be difficult.
  Depending on the SIMD instructions your CPU has available, the compiler might have some tricks up its sleeve using "masked" SIMD operations that can behave differently for different values.
  Still, the more branching your code has the less likely it is that auto-vectorization will work.
...
* **The potential for memory being non-contiguous.**
  This is particularly an issue with NumPy views, which can skip every N bytes for a variety of reasons, for example a slice across a dimension that doesn't match the data order in memory.
  Code that operates on memory that may or may not be contiguous will get optimized differently by the compiler compared to code that can assume the data is contiguous.

These issues can affect performance in additional ways, like preventing compiler optimizations and instruction-level parallelism.
So avoiding these issues is a good idea in general, and they are mostly covered in other chapters in the book.
...
## Use SIMD as an alternative to explicit branchless code

In the previous chapter you learned about avoiding branches using branchless code.
If you are using auto-vectorization, the compiler _might_ be able to optimize branches out of existence, using "masked" operations where the SIMD instruction can be applied to only some of the data.
That is, it will automatically convert seemingly-branched code into being branchless without you having to change the code.
If you're using less automatic methods to generate SIMD code, as discussed in the next chapter, you can also sometimes explicitly use these so-called "masked" SIMD operations to get rid of branches.
...
Different CPUs might support different SIMD instructions.
This is not a problem for Numba, because compilation happens the first time a function is called.
That means Numba knows exactly which CPU you're using, and can compile accordingly.
Other programming languages are typically compiled in advance, so they can't just check the runtime CPU.

This and other SIMD difficulties and approaches are discussed in the next chapter.
The chapter after that briefly discusses the impact of specific CPU models on performance.
On a first reading of the book you may wish to skip these chapters, and jump straight ahead to the chapter on [memory caches](060_memory.qmd), a very different kind of hardware that can impact your code's performance.
