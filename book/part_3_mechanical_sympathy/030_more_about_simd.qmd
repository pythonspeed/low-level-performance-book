...
1. **Auto-vectorization:** As you saw in the last chapter, compilers can sometimes generate SIMD instructions automatically, especially if you structure your code appropriately.
...
2. **Intrinsics:** You can directly tell the compiler to use specific CPU instructions, using special compiler features called "intrinsics".
   ...
   Even if you're targeting a single architecture, different x86-64 CPU models support different instructions, as I'll discuss next.
...
For example, if you're compiling C/C++/Cython extensions for Python, by default no auto-vectorization happens at all.
...
::: {.callout-note}
While SLP is on by default in Clang, at the time of writing Numba has disabled SLP due to issues with bad code generation.
...
The next chapter briefly discusses the impact of specific CPU models on performance.
On a first reading of the book you may wish to skip this chapters, and jump straight ahead to the chapter on [memory caches](060_memory.qmd), a very different kind of hardware that can impact your code's performance.
