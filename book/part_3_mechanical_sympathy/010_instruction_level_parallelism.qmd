...
Your first though when you see the word "parallelism" might be using multiple threads or processes, to take advantage of multiple CPU cores.
But that's not what I'm talking about in this chapter; I'll cover that elsewhere, in [The Practice of Parallelism](../part_4_parallelism/000_what_is.qmd).
Rather, I'm talking about the CPU's ability to implement parallelism within a single CPU core, in a single thread—if your code structure allows it.
...
# other effects; we'll talk about SIMD in the next chapter.
import os
...
I expect the result of calling `pythagorean_theorem(3, 4)` to be 5.
When thinking about correctness, I don't care how the CPU executes the resulting code: so long as I get the correct result, faster is better.
But when it comes to performance, it's helpful to understand at least a little about how the code is actually executed.

A common initial mental model of CPU cores is that they execute each instruction in order.
In the case of `pythagorean_theorem()`, that would mean first a multiply, then another multiply, then an addition, then whatever instructions are necessary to calculate the square root:

::: {.content-hidden when-format="markdown"}
```{mermaid}
flowchart TD
 A("x_sq = x_length ** 2") --> B("y_sq = y_length ** 2")
 B --> C("__temp = x_sq + y_sq")
...
To write faster code, you need a more accurate mental model of how CPUs work; it will still be imperfect, but closer to reality.
In these model, modern CPUs can run your code more quickly by running multiple instructions in parallel—so long as that won't affect the result.
...
In this case, calculating `x_sq` and `y_sq` is completely independent, so your CPU is likely to run both at once.
This happens transparently: the compiler doesn't have to do anything special, the CPU will do this all on its own.

::: {.content-hidden when-format="markdown"}
```{mermaid}
flowchart TD
 A("x_sq = x_length ** 2") --> C("__temp = x_sq + y_sq")
 B("y_sq = y_length ** 2") --> C
 C --> D("... result = sqrt(__temp) ...")
```
:::
...
::: {.content-hidden unless-format="markdown"}
```
x_sq = x_length ** 2       y_sq = y_length ** 2
          \                                /
...
For one thing, the CPU can't run a calculation if its inputs aren't yet available:

* The CPU cannot run the instructions for `x_sq + y_sq` until both `x_sq` and `y_sq` have been calculated.
* Similarly, the CPU can't calculate the square root until `x_sq + y_sq` finishes.

These are "data dependencies" that prevent instruction-level parallelism (ILP), and therefore reduce the code's potential runtime speed.

Branches, as discussed in the next chapter, also interact with ILP.
And there are a variety of hardware limits tied to specific CPU models: a CPU core can only add a limited number of integers in parallel, for example, depending on the model.

## Example #1: Removing data dependencies with interleaving

To see the speed impact of ILP, consider the following example: generating an array of random numbers.
For educational purposes, I found [an example of how to do this](https://nuclear.llnl.gov/CNP/rng/rngman/node4.html) and wrote the following code:

```{python}
import numpy as np
from numba import uint32, uint64

# 64-bit linear congruent generator of a 32-bit pseudo-random number. The
# "uint32(uint32)" tells Numba the input is an unsigned 32-bit number as is the
# output.
@jit("uint32(uint32)")
def lcg(seed):
...
```{python}
#| echo: false
# Important to this after the NUMBA_SLP_VECTORIZE=0 etc.
%load_ext book_magics
```

To get a sense of the randomness, I will visualize the result as an image.
While a lack of patterns doesn't necessarily mean the data is random, if you do see patterns then it definitely is not random.
...
The result of `generate_random_numbers()` looks like noise, which is what one would want in a random number generator:
...
How much speed am I losing because of the data dependencies above?
To find out, I'll tweak the code to completely remove all data dependencies.
The result won't be useful code, but it will show how much of a speed-up I can potentially get.

I'm going to calculate `result[i]` purely based on `i`:
...
These calculations are completely unrelated and so a CPU core can calculate them in parallel:

* `result[3] = lcg(3)`
* `result[2] = lcg(2)`

This version of the code is not a good random number generator, as you can see when I visualize the results:
...
But this experiment does give a sense of how fast the random number generator could potentially go.
...
This performance experiment was informative, but the result is not useful as a random number generator.
But there are less naive approaches that should preserve the pseudo-randomness, while breaking the data dependency.

Instead of having a single random number generator, I'm going to have 4, and I'm going to interleave their calculations.
The result will be slightly different, but it's a random number generator, so that's fine.

For other algorithms you might be able to get the same kind of performance improvement by interleaving calculations from different areas of the data.
For example, in a row-oriented algorithm processing 2D data you could interleave the calculations for multiple rows.
...
The is resulting image is random-looking again:
...
In practice, I've told the compiler a write to `result[i * 4]` followed by `i += 1` followed by a read to `result[(i - 1) * 4]`, with a whole bunch of other array reads and writes in the middle.
...
To try to reduce the number of instructions, instead of reading back intermediate values from the array, I will explicitly store them in temporary variables.
This reduces the amount of arithmetic and array reads I'm asking the compiler to do, so it should be faster on that account.
And since the code I'm giving the compiler is simpler, it may also enable the compiler to do additional optimizations:
...
And now my code is much faster, with approximately the same number of instructions as the original version, but with significant parallelism:
...
Here's another data dependency bottleneck: an accumulator, summing an array:
```{python}
...
In every iteration of the loop the CPU add values to the same variable, `total`.
That variable acts as a data dependency, and prevents instruction-level parallelism:

This can be solved by having multiple partial accumulators, that get data added in parallel.
At the end of the function I add the accumulators to get the final sum:
...
And as I'd hoped, the second version allows the CPU core more scope for parallelism:
...
::: {.callout-note}
I'll discuss SIMD in the next chapter.
If you were to run the above code with SIMD enabled, the `sum1()` function might be faster than `sum2()`, or the same speed.
This technique is still worth trying even if SIMD is enabled, as it sometimes can help.
:::
...
As an equivalent to the `sum1()/sum2()` example above, here's a normal sum function written in Rust:
...
And here how you would write Rust code utilizing `iterator_ilp` to achieve the equivalent of the 4 accumulators I used in the `sum2()` example above:
...
What happens if your code has a branch, for example an `if/else` statement?
At first glance this would seem to prevent ILP: when the branch is hit, it's unclear which of two sets of instructions the CPU should execute, the ones in the `if` clause or the ones in the `else` clause.
However, to maximize performance, CPUs will heuristically guess which branch the running program will take and speculatively execute those instructions.
This can lead to faster code—or, as you'll learn in the next chapter, slower code.
