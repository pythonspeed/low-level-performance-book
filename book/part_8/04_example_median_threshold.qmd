# Example: A specialized algorithm can be much faster
...
The generic median implementation Numba provides is likely to be fairly generic, since it needs to work in a wide variety of circumstances.
...
We're going to implement a histogram-based median, as we did in an earlier chapter, based on the fact we're using 8-bit images that only have a limited range of potential values.
The median is the value where 50% of the pixels' values are smaller, and 50% are bigger.

::: {.callout-note}
While we are using the same technique, you should keep reading.
In the next step we'll build on this to code use new techniques that haven't been covered so far.
:::
...
For example, given a 0-255 range, if the median is going to be 10 we want to start from the smallest bucket to minimize additions.
