...
def naive_filter_python(arr1, arr2):
    ...
    # A simple slice of a NumPy array doesn't copy the data, it just points at
...
@njit
def naive_filter_numba(arr1, arr2):
    # This is the same code as naive_filter_python() above, but it will be compiled
    # to machine code the first time it is called with particular types.
    result = np.empty(shape=arr1.shape, dtype=arr1.dtype)
...
assert np.array_equal(
    naive_filter_python(SMALL_DATA, SMALL_DATA2),
    naive_filter_numba(SMALL_DATA, SMALL_DATA2)
)
```
...
%%compare_timing
naive_filter_numba(SMALL_DATA, SMALL_DATA2)
naive_filter_numba(LARGER_DATA, LARGER_DATA2)
```
...
Now, let's see how all three of our implementations compare.
...
timings = benchit.timings(
    [naive_filter_python, naive_filter_numba, smarter_filter_python],
    inputs,
    multivar=True,
...
Here's what the resulting run time looks like:

```{python}
timings.plot(logx=True, logy=True, dpi=100, figsize=(8, 4))
```

**Once inputs are large enough, the linear algorithm in Python is faster than the compiled but quadratic Numba version.**
That's why it's so important to pick a scalable algorithm: a non-scalable algorithm will overwhelm any benefits you get from switching to a faster programming language, once your inputs are large enough.
