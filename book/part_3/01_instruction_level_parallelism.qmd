...
def generate_random_numbers(n):
    ...
    random_number = uint64(1)
    for i in range(n):
        random_number = lcg(random_number)
        result[i] = random_number >> 32
    return result
```
...
We can preserve the pseudo-randomness, while breaking the data dependency, a different way: by doing two different calculations that don't share a data dependency.
...
def generate_random_numbers_3(n):
    ...
    for i in range(n // 2):
        ...
        result[i * 2] = random_number1 >> 32
        random_number2 = lcg(random_number2)
        result[i * 2 + 1] = random_number2 >> 32
...
We now have more opportunity for parallelism, as the two code paths in each loop iteration are independent of each other.
