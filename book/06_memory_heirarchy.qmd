...
Each of the values in the example above is a 32-bit integer, i.e. it takes 4 bytes.
So if we're loading 64 bytes at a time, we'll also load the 2nd, 3rd, 4th up to the 15th consecutive value into the L1 cache.

Comparing our two variations:

* **Linear scan:** Each load of an integer also loads the next 15 consecutive integers.
  This is helpful, since we're doing a linear scan, so we're going to be inspecting these integers next.
* **Random scan:** Each load of an integer also loads the next 15 consecutive integers, but they're of no immediate use to us since the next value we'll inspect will be elsewhere in the array.
...
## Registers: even faster than the L1 cache

The diagram above of the memory hierarchy leaves out one form of memory that is even faster than the L1 cache: CPU registers.
The registers are where the CPU can store temporary values that it is running instructions on.
Registers typically store at most a 64-bit value; SIMD is implemented with special registers that can store 128, 256, or 512-bit values.

Register are used as part of the way function calls are implemented, for arguments to functions and the return result of a function.
As a simplification, you can assume local function variables of 64 bits or less are stored in registers: numbers, and maybe other small values depending on the programming language.
For our purposes, that means it's faster to read and write numbers from a local variable than it is to and from memory.

```{python}
@njit
def multiply_in_array(arr):
    for i in range(1_000):
        arr[0] *= 1.1

@njit
def multiply_with_local(arr):
    temp = arr[0]
    for i in range(1_000):
        temp *= 1.1
    arr[0] = temp
...
#| echo: false
%%compare_timing
ARR[0] = 1; multiply_in_array(ARR)
ARR[0] = 1; multiply_with_local(ARR)
```
