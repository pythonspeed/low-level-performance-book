...
In the examples in this book I've been testing that optimized code has the same result as the original code.
But usually I only use just one input, or at most a handful of different inputs; I am writing code optimized for educational purposes.
...
Because NumPy appears to be turning `total = 0` into a `np.float64`, whereas NumPy is doing all the math with `np.uint64`.

```{python}
import numpy as np

python_int = 1
np_uint64 = np.uint64(1)

print(type(python_int + np_uint64))
```

### Next steps {.unnumbered}

In addition to making sure you code doesn't break, you also want to make sure it's maintainable.
