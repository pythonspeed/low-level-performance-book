...
The measured speed for `calc_fixed()` gives misleading results if the goal is to benchmark the wrapped `calc()`, likely because extra compiler optimizations can be applied for constant inputs.
...
The less noise your microbenchmark has, the more reliably you can see if small optimizations made a difference.
For example, if you have a ±10% variation in speed, you won't be able to reliably observe a 5% speed improvement, so you might reject a useful optimization to your code.

You will likely start by fixing large performance problems, so a noisy microbenchmark might be fine initially.
But solving small sources of slowness is also useful, as I'll discuss next, so reducing the noise from microbenchmarks can be very useful in the long run.
