...
In some compiled languages the same compiler functionality that generates specialized code for functions can also generate specialized data structures.
Consider a hash map, which internally maps objects to integers using a _hashing function_.

Python's hash map, the `dict` type, supports keys that can be any Python object.
Therefore to calculate the hash of the object, the Python interpreter first needs to lookup which specific hash function to use for this particular type of object, and do this over and over for every single key.

In contrast, in Rust the `HashMap` class is _generic_: you need specify the types of the keys and values, and the compiler generates a custom copy of the code depending on those types.
If you create a `HashMap<i32, i64>`, the compiler knows the keys are only ever 32-bit signed integers, and it can generate the hashing code appropriately.
This combines well the fact the compiler then optimizes the generated code, as I will discuss in later chapters.
