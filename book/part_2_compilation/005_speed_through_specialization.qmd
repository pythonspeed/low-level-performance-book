...
rustimport.settings.cache_dir = "frequent_rs/target/rustimport/"

# Don't output from Rust compiler:
import os
os.environ["RUSTFLAGS"] = "--quiet"
```
...
## Use a compiled language for calculations

```{python}
assert frequent_py(DATA) == frequent_rs.naive(DATA)
```
...
%%compare_timing
frequent_py(DATA)
frequent_rs.naive(DATA)
```

## Use static typing to generate a specialized data structure

but also because all the math incrementing counters doesn't have Python overhead at all.
But we can do better.

```{python}
assert frequent_py(DATA) == frequent_rs.python_iterator(DATA)
```
...
%%compare_timing
frequent_py(DATA)
frequent_rs.python_iterator(DATA)
```
