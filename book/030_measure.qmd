...
But there are different measures you can use, depending on your use case, which can lead you in different directions.

## An example: Geolocating

Here's an example: imagine you're working on a geocoding service, that turns addresses into latitude/longitude pairs.
...
To measure these two APIs, you could send a few million requests to the API server as fast as possible, and also create an equivalent CSV file for the batch API.
...
You could measure elapsed time, just keeping in mind the number of requests you sent.
And that's probably fine for individual optimization session, but it does suffer as a long-term metric.
If you change the input size in your benchmarks, the results won't be comparable.

### Throughput

A better metric is throughput: addresses/second.
...
And if you switch to uploading a CSV with 1 million addresses instead of 2 million addresses, the throughput should still be comparable between the two CSVs.

But for the individual API, this metric is still insufficient.
