...
The LLVM compiler used by Numba is smart enough to figure out that the sum of 0, 1, 2, ..., N is actually `N×(N + 1)/2`, so it just substitutes the faster formula when it can.
