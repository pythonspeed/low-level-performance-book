...
* Reading and writing values to/from a hash map like Python's `dict` type is an amortized $O(1)$: on average it's constant time.
  In practice most of the time you can ignore the "amortized" part, as we will in this chapter.
* Reading and writing values to/from indexes in a vector like Python's `list` type is $O(1)$: it's constant time.
