...
As an example, consider two commonly used data structures:
...
To read or write from a key, a `dict` hashes the key into an integer, maps that to an internal vector within the `dict`, and then does an equality comparison.
...
This is of course just an example, but the general principle applies more broadly: there are often faster alternatives that won't compromise your scalability.
But how do you find these alternatives?
