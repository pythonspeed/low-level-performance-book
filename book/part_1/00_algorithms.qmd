...
As explained in the introduction, this book focuses on situations where the performance bottleneck is computation.
Given that assumption, there are two high-level reasons the code might be slow:

1. Which algorithm is being used.
...
Depending on the algorithm's scalability, it might take 3× as long, or 1,000× as long, or even 1,000,000× as long.
...
# 😎 This is approximately the same code as naive_filter_python() above. It
...
Let's compare `naive_filter_compiled()`—implemented in a fast language (Numba)—to `smarter_filter_python()`, which is written in a slow language (Python).
