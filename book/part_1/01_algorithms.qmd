...
2. The implementation.
   For example, as we'll be repeatedly repeating, low-level languages like C usually run the same algorithm much faster than Python can.

In the rest of the book we'll focus on the implementation part, but in this chapter we'll focus on the choice of algorithm.
...
NumPy's core data structure, the `ndarray`, is a bit like a standard Python list, except that `ndarray`s:
...
The following example shows a naive implementation of a quadratic algorithm that filters an array down to only those items that are present in the second array.
