...
Put more formally, if your algorithm is $O(n)$, you can model the performance with the formula $H·I·n$, where $H$ is a constant tied to specific hardware, and $I$ is a constant tied to a specific implementation.
Your goal is to try to switch from $I$ to a smaller constant $J$ by making the code more efficient.
The code's scalability doen't change when you change from $H·I·n$ to $H·J·n$: doubling $n$ will still double the run time.
...
```{python}
# 😎 Remember that by using the @jit decorator, our Python code gets compiled
...
def no_allocate_in_loop(arr):
    ...
    for i in range(len(arr) // 4):
        ...
        temp[:] = slice_of_4
        # 😎 Again, these operations are in-place, so they don't allocate:
        temp *= slice_of_4
        total += temp.mean()
    return total
