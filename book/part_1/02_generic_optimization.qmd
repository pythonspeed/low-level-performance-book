...
Put another way, if your algorithm is $O(N·log(N))$, you can model the performance as being $k·N·log(N)$, and your goal is to try to shrink the constant $k$.
...
That means you will almost always end up writing or call functions with the following basic structure:
...
When we rule out multiples of 5, we're ruling out `10 = 5 * 2` and `10 = 5 * 3` and ` 10 = 5 * 4` even though we've already ruled them out in previous rounds.
For example, `10 = 2 * 5` would have been previously ruled out as a multiple of 2.
...
Continuing the same example, in our loop above we do an `if is_prime[i]: continue` check on all even values of `i`, even though we know that any even number larger than 2 isn't prime.
Instead of doing this check in every single iteration, including odd numbers, we can split processing into two loops, one for odd numbers and one for even numbers.
...
def find_primes_3(length):
    ...
    # 😎 Unconditionally rule out all even numbers >2:
...
So we can optimize the code even further by doing every second multiple:
...
However, larger values that need to persist beyond the lifetime of the function call need to be allocated from the operating system, which is a more expensive operation.
...
* Mathematical operations on arrays can create new arrays, thus allocating memory.
  For example, `myarr + 1` creates a new array with the value at index `i` set to `myarr[i] + 1`.
