...
* This book uses a microbenchmarking library I wrote myself, wrapping Pytho's `timeit`.
...
This is slow, and it's a one time cost, so you usually want to ignore this compilation time when benchmarking: after all, it doesn't reflect the speed of the function.
...
The first time we call `arr_sum()` with an array of `np.int64`, the elapsed time will be much higher than subsequent times, due to compilation time:
