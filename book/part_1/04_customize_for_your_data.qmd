...
Another potential optimization—thought not the only one—is relaxing the precision or even accuracy of your code.
...
Given that range of values, we're not going to be able to use a small number of buckets if we have a bucket for each value.
And using a very large number of buckets will potentially be a lot slower, since we'll need to do a lot more addition to find the median.
