...
* Create a histogram with one bucket per possible pixel value; since we know our image is 8-bit, we only need 256 buckets.
* Iterate over each pixel in the chosen neighborhood.
* Each pixel's value will increase the count in the corresponding bucket in the histogram.
...
Looking at this histogram, I can estimate by eye that the point where half the pixels are on one side and half on the other, which is to say the median, will be somewhere between 140 and 160.
We can calculate it to validate that:
