...
We want to turn it into an image with only two colors, black and white.
...
median_local_threshold1(IMAGE, 11, 10)
```
...
How do I know that?
For one thing, the needs to look at all the neighbors of any given pixel; for a 11×11 neighborhood, that's 121 pixels to look at, which seems like it would be far more expensive than the final thresholding once we find the median.
For another, the [documentation for `numpy.median()`](https://numpy.org/doc/stable/reference/generated/numpy.median.html) talks about how by default it allocates a new array for every call, and we've already talked about avoiding allocations in the inner loop.
You can certainly validate my assumption with a profiler.

Now, the generic median implementation Numba provides is likely to be fairly generic, since it will need to work in a wide variety of circumstances.
...
def median_local_threshold2(img, neighborhood_size, offset):
    ...
    # 😎 A histogram with a bucket for each of the 8-bit values possible in the
...
#| echo: false
%%compare_timing
median_local_threshold1(IMAGE, 3, 10)
median_local_threshold1(IMAGE, 21, 10)
median_local_threshold2(IMAGE, 3, 10)
median_local_threshold2(IMAGE, 21, 10)
```
...
#| echo: false
%%compare_timing
median_local_threshold1(BLACK_IMAGE, 11, 10)
median_local_threshold1(WHITE_IMAGE, 11, 10)
median_local_threshold2(BLACK_IMAGE, 11, 10)
median_local_threshold2(WHITE_IMAGE, 11, 10)
```
