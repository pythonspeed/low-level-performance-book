...
The syntax for these operations looks like you're operating on a single value, but it's actually operating on a whole array:
...
def numpy_rescale_optimized(img, min_value, max_value):
    ...
    # 😎 While we now have whole number results, want the returned array to be
    # uint8, and `scaled` is float64. So we copy the data into `temp_arr`,
