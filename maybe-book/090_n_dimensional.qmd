...
## Make sure the compiler can take advantage of contiguous arrays
...
By default, Numba is smart enough to compile multiple versions of the function for contiguous and non-contiguous inputs, and use the appropriate one to maximize speed.
...
Unlike my previous version, the number of CPU instructions run by the new `scan_y_first_nc()` and `scan_x_first_nc()` are essentially identical.
There is still a significant performance gap between the two, likely the result of the memory access pattern:
...
In other programming languages you will need to explicitly do so if you want to get faster results for contiguous arrays.
For example, if you're using Cython to interface with NumPy arrays, my [article on Cython auto-vectorization](https://pythonspeed.com/articles/faster-cython-simd/) shows how to support both contiguous and non-contiguous arrays.
